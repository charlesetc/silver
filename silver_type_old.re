...
let rec string_of_typed_tree tree => {
  ...
  | Function_call ty tree1 tree2 =>
    "(" ^
      " " ^
      string_of_typed_tree tree1 ^
      " $ " ^
      string_of_typed_tree tree2 ^
      " " ^
      ") : " ^
      string_of_silver_type ty
...
let rec convert_to_typed_tree table tree => {
  ...
  let infer_literal_type table str => {
     ...
     * their own type.
     */
    let looks_like_integer str :bool =>
  ...
  | Silver_parse.Symbol (token, position) =>
    ...
    | Silver_token.Identifier str =>
      Symbol {silver_type: infer_literal_type table str, position, data: str}
    | Silver_token.String_literal str => Symbol {silver_type: String, position, data: str}
    | _ =>
      raise (
        Silver_bug
          (Printf.sprintf "cannot type-infer with token %s" (Silver_token.string_of_token token))
          position
      )
    }
    ...
    | Symbol (token, position) =>
      ...
      | Identifier data =>
        ...
        if (List.length arguments == 1) {
          Function_definition
            (generic_type ())
            {silver_type: type_of_argument, position, data}
          ...
          Function_definition
            ...
            {silver_type: type_of_argument, position, data}
            [
              convert_to_typed_tree
                table (Silver_parse.Lambda_list (List.tl arguments) abstract_trees)
            ]
        }
      | _ =>
        raise (
          Silver_utils.Silver_error
            (
              Printf.sprintf
                "found token %s, which is not an identifier, as an argument to a function definition"
    ...
    | false_argument =>
      raise (
        Silver_utils.Silver_error
          (
            Printf.sprintf
              "found tee %s, which is not an identifier, as an argumnt to a function definition"
...
let get_type tree =>
  switch tree {
  | Symbol t_unit => t_unit.silver_type
  | Function_call a_type function_itself argument => a_type
  | Function_definition a_type t_unit trees => a_type
  };
  ...
  List.map
    (
      fun (a, b) => print_string (
        "\t" ^ string_of_silver_type a ^ " == " ^ string_of_silver_type b ^ "\n"
      )
    )
    constraints;
...
let rec collect_constraints tree :list 'a => {
  let output = ref [];
  let constrain argument => output := [argument, ...!output];
  switch tree {
...
let apply_to_tree substitutions tree => {
  ...
  let rec apply_tree (x, typ) tree =>
    switch tree {
    | Symbol tunit => Symbol (apply_typed_unit tunit (x, typ))
    | Function_call ptype t1 t2 =>
      Function_call (substitute x typ ptype) (apply_tree (x, typ) t1) (apply_tree (x, typ) t2)
...
let rec unify constraints => {
  let rec unify_one t1 t2 =>
    ...
    | (Function a b as z, Generic x)
    | (Generic x, Function a b as z) =>
      if (occurs x z) {
